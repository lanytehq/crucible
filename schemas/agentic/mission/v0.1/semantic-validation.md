...
| SEM-A05 | Every mutating control request has a caller-stable idempotency key, a canonical `request_fingerprint`, and an `original_result_hash`. Exact replay is the same key and body: `request_id` is per-call correlation and is omitted from both digests along with the two hash fields. Embedded `request` and `result` must be schema-valid mission-control documents, share `request_id`, and digest to the stored hashes. One key maps to one fingerprint and one original result. Each `cancel_requested` evidence ref must name that binding. |
