...
| SEM-A05 | When a mutating control record is present (`mission.create`, `launch`, `close`, `cancel`), it has a caller-stable idempotency key, a canonical `request_fingerprint`, and an `original_result_hash`. Histories need not carry create/launch/close envelopes. Exact replay is the same key and body: `request_id` is per-call correlation and is omitted from both digests along with the two hash fields. Embedded `request` and `result` must be schema-valid, share `request_id`, digest to the stored hashes, and each present mission id must equal the enclosing history. One key maps to one fingerprint and one original result. Each `cancel_requested` evidence ref must name a `mission.cancel` binding. |

### Control hash bytes

Both `request_fingerprint` and `original_result_hash` are SHA-256 of the
UTF-8 encoding of the envelope after dropping `request_id`,
`request_fingerprint`, and `original_result_hash` at the top level. Remaining
JSON is serialized with object members sorted by Unicode code point of the
key, no insignificant whitespace (`:` between key and value, `,` between
members and array elements), RFC 8259 strings (unescaped Unicode except `"`,
`\`, and U+0000–U+001F), JSON `true`/`false`/`null`, and integers as base-10
without a sign prefix for zero, leading zeros, or exponent. The digest is
lowercase hex. Rust consumers must match this byte domain, not a language
`json.dumps` dialect.
